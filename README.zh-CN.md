...
基于 OKX WebSocket + LLM Agent 的数字货币智能监控系统，支持飞书/微信/QQ 自然语言交互与精确命令双模式。
...
- **多平台消息**：通过飞书/微信/QQ 命令添加/删除/查询监控规则（基于 larky 统一 API）
...
1. Redis 服务已运行
2. larky 统一消息服务已运行（`uv run python -m larky`）
...
- **无状态设计**：每次 `/ask` 调用是独立的，不保留对话历史，匹配 IM 平台的消息驱动模型。
...
| 无状态 Agent | 是 | 匹配 IM 平台消息驱动模型；每次 `/ask` 独立处理，避免会话状态管理复杂度 |
